...
- `Fix` Android 테스트에서 테스트 중에 다른 앱으로 이동하는 경우에 요소를 찾지 못하는 경우가 있어 해당 문제 수정하였습니다. 

### 브라우저 테스트

- `New` SauceLabs의 테스트 결과 화면에 Selenium 로그가 표시되도록 하였습니다.
- `New` 「모바일 에뮬레이션」테스트 실행 설정에서 「디바이스의 방향」지정 가능하게 하였습니다.
- `Fix` 「클라이언트 증명서」를 지정하여 테스트를 실행하면 화면 캡처 취득에 실패하는 경우가 있어 해당 문제 수정하였습니다.
- `Fix` 「클라우드」환경에서 브라우저 기동 시에 올바른 URL이 열리지 않는 경우가 있어 해당 문제 수정하였습니다.
